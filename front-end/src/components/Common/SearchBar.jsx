...
export const SearchBar = () => {
  ...
  return (
    <div className={`flex items-center justify-center w-full transition-all duration-300 ${isOpen? "absolute top-0 left-0 w-full bg-white h-24 z-50" :"w-auto"}`}>
     {isOpen?(
        <form role="search" aria-label="Site search" className='relative flex items-center justify-center w-full' onSubmit={handleSubmit}>
          <div className='relative w-1/2'>
          <input
            type="text"
            name="q"
            ...
            className='bg-gray-100 px-4 py-2 pl-2 pr-12 rounded-lg focus:outline-none w-full placeholder:text-gray-700'
          />
          <button type='submit' aria-label='Submit search' className='absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-600 hover:text-gray-800'>
            <HiMagnifyingGlass className='h-6 w-6'/>
          </button>
          </div>
        ...
        <button type='button' onClick={handleSearchToggle} aria-label='Open search'>
          <HiMagnifyingGlass className='h-6 w-6'/>
        </button>
      )
     } 
